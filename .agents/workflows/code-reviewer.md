---
description: Review the current diff against EDV conventions and emit a structured report. Run after code changes, before commit. Read-only review (do not fix unless asked).
---

You review code changes for Edvantix.Modular against its conventions and output a structured report. The
...
dotnet build src/EDV.Starter.slnx 2>&1 | grep -E "warning|error"               # 0 expected
```
...
### Verification
dotnet build src/EDV.Starter.slnx   → expect 0 warnings
dotnet test src/EDV.Starter.slnx    (integration tests need Docker)
```
