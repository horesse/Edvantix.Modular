...
description: CQRS interface reference for EDV. This project uses the Mediator source generator, NOT MediatR. Reference when implementing commands, queries, and handlers.
user-invocable: false
---

# Mediator Reference

⚠️ **EDV uses the `Mediator` source-generator package (`using Mediator;`), NOT `MediatR`.** Different
interfaces — MediatR types won't compile. The CQRS interfaces below are the library's own (no EDV wrapper).
...
list **plus** an entry in the `moduleAssemblies` array — in **both** `EDV.Starter.Api/Program.cs` **and**
`EDV.Starter.DbMigrator/Program.cs`:
...
builder.Services.AddMediator(o =>
{
    ...
    o.Assemblies = [
        /* … */
        typeof(EDV.Modules.{X}.Contracts.{X}ContractsMarker),   // Contracts assembly
        typeof(EDV.Modules.{X}.{X}Module)];                     // runtime assembly
});
```
