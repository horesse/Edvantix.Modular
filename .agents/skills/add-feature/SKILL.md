---
name: add-feature
description: Add a vertical-slice feature (command/query + handler + validator + endpoint) to an existing EDV module. Use when adding an API endpoint or business operation to a module that already exists.
...
```csharp
namespace EDV.Modules.{X}.Contracts.v1.{Area};
...
(`EDV.Framework.Shared.Persistence`) — see `query-patterns`.
...
Throw `NotFoundException` / `CustomException(msg, errors, HttpStatusCode)` (`EDV.Framework.Core.Exceptions`) — the global handler maps them to ProblemDetails.
...
```bash
dotnet build src/EDV.Starter.slnx          # 0 warnings (TreatWarningsAsErrors)
