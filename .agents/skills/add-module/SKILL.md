...
## Step 1 — `[Module]` is an ASSEMBLY attribute (not class-level)

In `{Name}Module.cs`, above the namespace:

```csharp
[assembly: Module(typeof(EDV.Modules.{Name}.{Name}Module), 900)]   // (Type, order)

namespace EDV.Modules.{Name};
...
public sealed class {Name}Module : IModule
{
    public void ConfigureServices(IHostApplicationBuilder builder)
    {
        ...
        builder.Services.AddDbContext<{Name}DbContext>();
...
```bash
dotnet sln src/EDV.Starter.slnx add src/Modules/{Name}/Modules.{Name}/Modules.{Name}.csproj
dotnet sln src/EDV.Starter.slnx add src/Modules/{Name}/Modules.{Name}.Contracts/Modules.{Name}.Contracts.csproj
```

Add a `<ProjectReference>` to the runtime module from **both** `EDV.Starter.Api` and `EDV.Starter.DbMigrator`, and reference the runtime project from `EDV.Starter.Migrations.PostgreSQL`.

## Step 5 — Migrations folder

Add a `{Name}/` folder in `src/Host/EDV.Starter.Migrations.PostgreSQL`, then create the initial migration (see **create-migration**) with `--context {Name}DbContext`.

## Step 6 — ⚠️ Register in ALL FOUR places (the footgun)

Identical edits in **both** `EDV.Starter.Api/Program.cs` **and** `EDV.Starter.DbMigrator/Program.cs`:

1. Mediator `o.Assemblies` — add **two** markers: a Contracts type (e.g. `typeof(EDV.Modules.{Name}.Contracts.{Name}ContractsMarker)`) **and** the module type (`typeof({Name}Module)`).
...
## Step 7 — Verify[SKILL.md](../add-permission/SKILL.md)

```bash
dotnet build src/EDV.Starter.slnx                  # 0 warnings
dotnet test src/Tests/Architecture.Tests           # boundary + tenant-isolation rules must pass
dotnet test src/EDV.Starter.slnx
```

## Checklist

- [ ] Two projects (copied csproj), added to `.slnx`, referenced from Api + DbMigrator (+ Migrations)
- [ ] `[assembly: Module(typeof({Name}Module), order)]` (assembly-level, positional)
- [ ] `IModule`: `AddDbContext<T>()`, `PermissionConstants.Register`, version-set group, eventing trio if needed
