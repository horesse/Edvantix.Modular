...
Login `POST /api/v1/identity/token/issue` with header `X-EDV-App: "admin"|"dashboard"`. **localStorage keys are namespaced per app** (`edv.admin.*` / `edv.dashboard.*`) so both run side-by-side. Permission *source* differs per app — see the app files.
...
- **JWT seeding:** `seedAuthedSession(page, TEST_USER)` builds a fake JWT and `addInitScript`-writes `edv.{app}.*` to localStorage before React boots (server isn't called, so signature is junk).
