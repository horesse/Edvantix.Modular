...
Each module implements `IModule`, declared via an **assembly-level** `[Module]` attribute (positional `(Type moduleType, int order = 0)`) — **not** a class-level `[Module(Order = n)]`:

```csharp
[assembly: Module(typeof(EDV.Modules.Identity.IdentityModule), 1)]   // above the namespace
...
`ModuleLoader.AddModules` (`src/BuildingBlocks/Web/Modules/ModuleLoader.cs`) discovers `[Module]` attributes, orders by `Order` then name, instantiates each, and calls `ConfigureServices`. Endpoints map under `api/v{version:apiVersion}/{module}`.
...
|---|---|---|
| Mediator `o.Assemblies` (two markers: Contracts type **and** module type) | `src/Host/EDV.Starter.Api/Program.cs` | Handlers silently undiscovered |
| `moduleAssemblies` array | `src/Host/EDV.Starter.Api/Program.cs` | Module never loaded |
| Mediator assemblies (same pair) | `src/Host/EDV.Starter.DbMigrator/Program.cs` | Migrate/seed misses the module |
| module assemblies array | `src/Host/EDV.Starter.DbMigrator/Program.cs` | Migrate/seed misses the module |
...
In `src/BuildingBlocks/Web/Extensions.cs` (`UsePlatform`):
...
`app.UseMultiTenantDatabases()` (Finbuckle `UseMultiTenant()`) runs in `Program.cs` **before** `UsePlatform`, i.e. **before `UseAuthentication`** — so tenant resolution is header-driven, not claim-driven. See `modules/multitenancy.md`.
...
- `appsettings.json` (+ `.Development`/`.Production`) live in `src/Host/EDV.Starter.Api/`. DbMigrator links the same files.
...
- Platform composition is one call each: `builder.AddPlatform(o => { o.Enable... })` (DI) and `app.UsePlatform(...)` (middleware). Feature flags toggle Caching/Jobs/Mailing/Quotas/Sse/Realtime/OpenTelemetry/CORS/Idempotency.
