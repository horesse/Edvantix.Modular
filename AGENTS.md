...
| `src/Host/EDV.Starter.Api` | Composition-root Web API host. |
| `src/Host/EDV.Starter.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, **both React apps**). |
| `src/Host/EDV.Starter.DbMigrator` | One-shot migrate/seed runner. DB is **not** migrated at API startup. |
| `src/Host/EDV.Starter.Migrations.PostgreSQL` | All EF migrations, organized per-module by folder. |
...
dotnet run --project src/Host/EDV.Starter.AppHost   # one-time: npm install in clients/admin & clients/dashboard

dotnet build src/EDV.Starter.slnx                   # build backend
dotnet run --project src/Host/EDV.Starter.Api       # API only → https://localhost:7030 (/scalar)
dotnet test src/EDV.Starter.slnx                    # tests — integration tests REQUIRE Docker
...
```bash
dotnet run --project src/Host/EDV.Starter.DbMigrator -- apply [--seed]
dotnet run --project src/Host/EDV.Starter.DbMigrator -- list-pending
```
...
- **Module** — new `Modules.{Name}` + `.Contracts`, implement `IModule` w/ assembly-level `[assembly: Module(typeof(XModule), order)]`, register in **all four places**, add migration folder + tests. Details: `architecture.md`.
