...
/// <remarks>
/// Помечен <see cref="ImmutableObjectAttribute"/> + <c>sealed</c>, чтобы HybridCache мог переиспользовать
/// внутрипроцессный экземпляр между запросами без повторной десериализации JSON-полезной нагрузки при
/// каждом попадании в L1. НЕ добавляйте изменяемые свойства и не делайте этот класс открытым для
/// наследования — это сломает оптимизацию переиспользования L1 в HybridCache.
/// </remarks>
...
{
    // Текущие URL (возвращаются из API)
    public string? LogoUrl { get; init; }
    public string? LogoDarkUrl { get; init; }
    public string? FaviconUrl { get; init; }

    // Загружаемые файлы (по той же схеме, что и фото профиля)
    ...
    // Флаги для удаления текущих ресурсов
