using EDV.Modules.Multitenancy.Contracts.Dtos;
using EDV.Modules.Multitenancy.Contracts.v1.UpdateTenantTheme;
using FluentValidation;
using System.Text.RegularExpressions;

namespace EDV.Modules.Multitenancy.Features.v1.UpdateTenantTheme;

public partial class UpdateTenantThemeCommandValidator : AbstractValidator<UpdateTenantThemeCommand>
{
    public UpdateTenantThemeCommandValidator()
    {
        RuleFor(x => x.Theme)
            .NotNull()
            .WithMessage("Тема обязательна.");

        RuleFor(x => x.Theme.LightPalette)
            .NotNull()
            .SetValidator(new PaletteValidator());

        RuleFor(x => x.Theme.DarkPalette)
            .NotNull()
            .SetValidator(new PaletteValidator());

        RuleFor(x => x.Theme.Typography)
            .NotNull()
            .SetValidator(new TypographyValidator());

        RuleFor(x => x.Theme.Layout)
            .NotNull()
            .SetValidator(new LayoutValidator());
    }

    [GeneratedRegex("^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{8})$")]
    private static partial Regex HexColorRegex();

    private sealed class PaletteValidator : AbstractValidator<PaletteDto>
    {
        public PaletteValidator()
        {
            RuleFor(x => x.Primary).Must(BeValidHexColor).WithMessage("Primary должен быть корректным hex-цветом.");
            RuleFor(x => x.Secondary).Must(BeValidHexColor).WithMessage("Secondary должен быть корректным hex-цветом.");
            RuleFor(x => x.Tertiary).Must(BeValidHexColor).WithMessage("Tertiary должен быть корректным hex-цветом.");
            RuleFor(x => x.Background).Must(BeValidHexColor).WithMessage("Background должен быть корректным hex-цветом.");
            RuleFor(x => x.Surface).Must(BeValidHexColor).WithMessage("Surface должен быть корректным hex-цветом.");
            RuleFor(x => x.Error).Must(BeValidHexColor).WithMessage("Error должен быть корректным hex-цветом.");
            RuleFor(x => x.Warning).Must(BeValidHexColor).WithMessage("Warning должен быть корректным hex-цветом.");
            RuleFor(x => x.Success).Must(BeValidHexColor).WithMessage("Success должен быть корректным hex-цветом.");
            RuleFor(x => x.Info).Must(BeValidHexColor).WithMessage("Info должен быть корректным hex-цветом.");
        }

        private static bool BeValidHexColor(string color) =>
            !string.IsNullOrWhiteSpace(color) && HexColorRegex().IsMatch(color);
    }

    private sealed class TypographyValidator : AbstractValidator<TypographyDto>
    {
        public TypographyValidator()
        {
            RuleFor(x => x.FontFamily)
                .NotEmpty()
                .MaximumLength(200)
                .Must(BeValidFontFamily)
                .WithMessage("FontFamily должен быть допустимым web-safe шрифтом.");

            RuleFor(x => x.HeadingFontFamily)
                .NotEmpty()
                .MaximumLength(200)
                .Must(BeValidFontFamily)
                .WithMessage("HeadingFontFamily должен быть допустимым web-safe шрифтом.");

            RuleFor(x => x.FontSizeBase)
                .InclusiveBetween(10, 24)
                .WithMessage("FontSizeBase должен быть в диапазоне от 10 до 24.");

            RuleFor(x => x.LineHeightBase)
                .InclusiveBetween(1.0, 2.5)
                .WithMessage("LineHeightBase должен быть в диапазоне от 1.0 до 2.5.");
        }

        private static bool BeValidFontFamily(string fontFamily) =>
            TypographyDto.WebSafeFonts.Contains(fontFamily);
    }

    private sealed class LayoutValidator : AbstractValidator<LayoutDto>
    {
        public LayoutValidator()
        {
            RuleFor(x => x.BorderRadius)
                .NotEmpty()
                .MaximumLength(20)
                .Matches(@"^\d+(px|rem|em|%)$")
                .WithMessage("BorderRadius должен быть корректным значением CSS (например, '4px', '0.5rem').");

            RuleFor(x => x.DefaultElevation)
                .InclusiveBetween(0, 24)
                .WithMessage("DefaultElevation должен быть в диапазоне от 0 до 24.");
        }
    }
}