...
using EDV.Modules.Identity.Contracts.v1.Groups.GetGroupById;
using EDV.Modules.Identity.Data;
using Mediator;
using Microsoft.EntityFrameworkCore;

namespace EDV.Modules.Identity.Features.v1.Groups.GetGroupById;
...
public sealed class GetGroupByIdQueryHandler : IQueryHandler<GetGroupByIdQuery, GroupDto>
{
    ...
    public async ValueTask<GroupDto> Handle(GetGroupByIdQuery query, CancellationToken cancellationToken)
    {
        var group = await _dbContext.Groups
            ...
            ?? throw new NotFoundException($"Группа с ID '{query.Id}' не найдена.");
