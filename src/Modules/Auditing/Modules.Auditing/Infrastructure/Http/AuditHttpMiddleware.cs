...
public sealed class AuditHttpMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        // Отказ на уровне эндпоинта через [NoAudit]. Два варианта:
        //   • полный пропуск (по умолчанию) — не писать аудит вообще
        //   • только тело — записать активность, но опустить превью
    ...
    // Потоковые ответы (SSE / SignalR SSE) никогда не должны буферизоваться: путь аудита буферизует Response.Body
    // и сбрасывает его только при возврате обработчика, но долгоживущий поток никогда не возвращается, так что клиент
    // зависнет. Определяем по заголовку Accept и пропускаем без изменений.
