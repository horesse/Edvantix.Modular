...
/// <summary>
/// Fluent-точка входа для создания и публикации событий аудита.
/// Настраивается один раз при старте: публикатор, сериализатор и опциональные обогатители.
/// </summary>
public static class Audit
{
    ...
    // Неизменяемый снимок, атомарно подменяемый в Configure(). Читатели делают одно volatile-чтение
    // и перебирают именно его — никогда не живую коллекцию — поэтому переконфигурация не может
    // вызвать исключение "Collection modified".
    ...
    public sealed class Builder
    {
        ...
        public async ValueTask WriteAsync(CancellationToken ct = default)
        {
            ...
            // Обогащение перед публикацией — одно volatile-чтение, затем перебор неизменяемого снимка.
    ...
    // --- минимальные безопасные значения по умолчанию для запуска dev-сборок ---
