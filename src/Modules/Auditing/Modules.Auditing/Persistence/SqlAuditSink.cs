...
/// <summary>
/// Сохраняет конверты аудита в SQL через EF Core.
/// </summary>
public sealed class SqlAuditSink : IAuditSink
{
    ...
    public async Task WriteAsync(IReadOnlyList<AuditEnvelope> batch, CancellationToken ct)
    {
        ...
        // Обрабатываем по арендатору, чтобы у MultiTenantDbContext был окружающий контекст арендатора.
        foreach (var group in batch.GroupBy(e => e.TenantId))
        {
            ...
            {
                _log.LogWarning("Пропуск записи аудита для арендатора {TenantId}, так как арендатор не найден.", group.Key ?? "<null>");
                continue;
            }
            ...
            {
                _log.LogInformation("Записано {Count} записей аудита для арендатора {TenantId}.", records.Count, tenantInfo.Id);
            }
        }
    }
}