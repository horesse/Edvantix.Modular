...
public class SmtpMailService(IOptions<MailOptions> settings, ILogger<SmtpMailService> logger) : IMailService
{
    ...
    private void ValidateSmtpConfiguration()
    {
        ...
        {
            throw new InvalidOperationException("SMTP Host не настроен.");
        }
    }
    ...
    {
        var fromAddress = request.From ?? _settings.From ?? throw new InvalidOperationException("Не настроен адрес отправителя.");
    ...
    private async Task SendEmailAsync(MimeMessage email, CancellationToken ct)
    {
        ...
        // Широкий перехват intentional: любая ошибка SMTP (аутентификация, сеть, протокол) логируется
        // и оборачивается в согласованное исключение для вызывающих.
        catch (Exception ex)
        {
            logger.LogError(ex, "Произошла ошибка при отправке письма: {Message}", ex.Message);
