...
public sealed class HangfireCustomBasicAuthenticationFilter : IDashboardAuthorizationFilter
{
    ...
    public bool Authorize(DashboardContext context)
    {
        ...
        {
            _logger.LogInformation("В запросе отсутствует заголовок Authorization");
        ...
        {
            _logger.LogInformation("Запрос НЕ использует базовую аутентификацию");
            SetChallengeResponse(httpContext);
            return false;
        }
        ...
        {
            _logger.LogInformation("Токены аутентификации недействительны (пустые, null или содержат только пробелы)");
            SetChallengeResponse(httpContext);
            return false;
        }
        ...
        {
            _logger.LogInformation("Отлично, токены аутентификации совпадают с конфигурацией!");
            return true;
        }

        _logger.LogInformation("Аутентификация панели Hangfire не удалась — учётные данные не совпадают с конфигурацией");
