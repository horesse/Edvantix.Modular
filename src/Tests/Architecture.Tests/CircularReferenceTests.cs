...
/// <summary>
/// Тесты для обнаружения циклических ссылок между проектами решения.
/// Циклические ссылки приводят к проблемам сборки и указывают на архитектурные проблемы.
/// </summary>
public class CircularReferenceTests
{
    ...
    public void Solution_Should_Not_Have_Circular_Project_References()
    {
        ...
        cycles.ShouldBeEmpty(
            $"Обнаружены циклические ссылки между проектами: {string.Join("; ", cycles)}");
    }
    ...
    public void Modules_Should_Not_Have_Circular_Dependencies()
    {
        ...
        cycles.ShouldBeEmpty(
            $"Обнаружены циклические зависимости между модулями: {string.Join("; ", cycles)}");
    }
    ...
    public void BuildingBlocks_Should_Not_Have_Circular_Dependencies()
    {
        ...
        cycles.ShouldBeEmpty(
            $"Обнаружены циклические зависимости между BuildingBlocks: {string.Join("; ", cycles)}");
    }
    ...
    public void Dependency_Graph_Should_Be_Acyclic()
    {
        ...
        // Пытаемся выполнить топологическую сортировку — при наличии циклов она не удастся
        _ = TopologicalSort(dependencyGraph, out var hasCycle, out var cycleDescription);

        hasCycle.ShouldBeFalse(
            $"Граф зависимостей не является ациклическим. {cycleDescription}");
    }
    ...
    private static HashSet<string> GetProjectReferences(string projectPath)
    {
        ...
        {
            // Игнорируем ошибки разбора XML
        }
        catch (IOException)
        {
            // Игнорируем ошибки ввода-вывода файлов
        }
        ...
        List<string> cycles)
    {
        ...
        {
            // Найден цикл — извлекаем его путь
        ...
        out string cycleDescription)
    {
        ...
        {
            cycleDescription = $"Обнаружен цикл в узле: {node}";
            return false;
        }
